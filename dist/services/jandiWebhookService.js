...
exports.sendJandiNotification = sendJandiNotification;
exports.sendSimpleJandiMessage = sendSimpleJandiMessage;
/**
 * 주간 리포트 알림 메시지 생성
 */
function buildWeeklyReportMessage(aggregation, spreadsheetUrl) {
    const { brandName, weekLabel, totalReviews, avgRating, sentimentDistribution, topKeywords, issueKeywords, negativeReviews, aiInsights, } = aggregation;
 ...
 * 잔디 웹훅 전송
 */
async function sendJandiNotification(webhookUrl, aggregation, spreadsheetUrl) {
    try {
        ...
        console.error('Jandi webhook error:', error.message);
        return false;
    }
}
/**
 * 간단한 알림 메시지 전송 (테스트용)
 */
async function sendSimpleJandiMessage(webhookUrl, title, message) {
    try {
        const payload = {
            body: title,
            connectColor: '#FAC11B',
        ...
        console.error('Jandi simple message error:', error.message);
        return false;
    }
}
//# sourceMappingURL=jandiWebhookService.js.map