...
import {
  ...
  generateBrandWeeklyReport,
  generateWeeklyReportsForDate,
...
export async function weeklyReportRoute(fastify: FastifyInstance) {
  /**
   ...
   */
  fastify.post<{ Querystring: GenerateReportQuery; Reply: GenerateReportResponse }>(
    '/api/weekly-report/generate',
    async (request, reply) => {
      const { brand, date } = request.query;

      try {
        let results: WeeklyReportResult[];
        ...
        if (brand && date) {
          ...
          const result = await generateBrandWeeklyReport(brand, fastify.log, targetDate);
          results = [result];
        } else if (brand) {
          // 지난 주의 특정 브랜드
          const result = await generateBrandWeeklyReport(brand, fastify.log);
          results = [result];
        } else if (date) {
          // 특정 날짜의 전체 브랜드
          const targetDate = new Date(date);
          ...
          results = await generateWeeklyReportsForDate(targetDate, fastify.log);
        } else {
          // 지난 주 전체 브랜드
          results = await generateWeeklyReports(fastify.log);
        }
