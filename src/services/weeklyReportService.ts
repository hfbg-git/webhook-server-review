...
import { WeeklyReportResult, WeekRange, AIInsights, BrandWeeklyAggregation } from '../types/index.js';
import { loadBrandCache } from './brandRegistry.js';
import { getNotificationConfig, loadNotificationConfigCache } from './notificationConfigService.js';
import { sendJandiNotification } from './jandiWebhookService.js';
 ...
 * 잔디 알림 전송 (설정된 브랜드만)
 */
async function sendJandiNotificationIfEnabled(
  brandName: string,
  ...
  try {
    const notificationConfig = await getNotificationConfig(brandName);

    if (notificationConfig && notificationConfig.enabled) {
      const notificationSent = await sendJandiNotification(
        notificationConfig.jandiWebhookUrl,
        aggregation,
        spreadsheetUrl
      );
...
export async function generateWeeklyReports(logger: Logger): Promise<WeeklyReportResult[]> {
  ...
  await loadBrandCache(true);
  logger.info({ msg: 'Brand cache loaded for weekly report generation' });

  // 알림 설정 캐시 로드
  await loadNotificationConfigCache(true);
