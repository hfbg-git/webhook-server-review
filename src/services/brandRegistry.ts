...
// 메인 브랜드 목록 (앞 3글자 기준)
export const MAIN_BRAND_PREFIXES = [
  '화락바',  // 화락바베큐치킨
 ...
 * 메인 브랜드 여부 확인
 */
export function isMainBrand(brandName: string): boolean {
  const prefix = brandName.replace(/\s/g, '').slice(0, 3);
  return MAIN_BRAND_PREFIXES.includes(prefix);
}

// 메모리 캐시 (서버 시작 시 로드)
let brandCache: Map<string, string> = new Map();
 ...
 * 공백 제거하여 정규화 키 생성
 */
function normalizeKey(brandName: string): string {
 ...
 * 앞 3글자 키 생성 (공백 제거 후)
 */
function getPrefix3Key(brandName: string): string {
 ...
 */
async function ensureRegistryTab(spreadsheetId: string): Promise<void> {
 ...
 */
async function findInRegistryByPrefix(
  spreadsheetId: string,
  prefix: string
): Promise<{ key: string; standard: string } | null> {
  const sheets = getSheetsClient();
 ...
 */
export async function loadBrandCache(forceReload = false): Promise<void> {
  ...
  try {
    ...
    console.warn('Failed to load brand registry, will use fallback:', (error as Error).message);
 ...
 * @returns 표준화된 브랜드명
 */
export async function getStandardBrandName(rawBrandName: string): Promise<string> {
 ...
 */
export async function seedInitialBrands(): Promise<void> {
  ...
  try {
    ...
    console.error('Failed to seed initial brands:', (error as Error).message);
  }
}
