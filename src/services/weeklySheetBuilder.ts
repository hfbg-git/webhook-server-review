...
import { isMainBrand } from './brandRegistry.js';
 ...
 */
async function getOrCreateBrandReportFolder(
  brandName: string,
  weekLabel: string
): Promise<string> {
  ...
  // 1. WeeklyReports 폴더
  const weeklyReportsFolderId = await getOrCreateFolder(
    WEEKLY_REPORT_FOLDER_ID,
    'WeeklyReports'
  );

  // 2. 메인/서브 브랜드 카테고리 폴더
  const categoryFolder = isMainBrand(brandName) ? '메인브랜드' : '서브브랜드';
 ...
 * 스프레드시트 생성
 */
async function createSpreadsheet(parentId: string, sheetName: string): Promise<string> {
  const drive = getDriveClient();
 ...
 * 시트 추가
 */
async function addSheet(spreadsheetId: string, title: string, index: number): Promise<number> {
 ...
 * 기본 Sheet1 삭제
 */
async function deleteDefaultSheet(spreadsheetId: string): Promise<void> {
  const sheets = getSheetsClient();
  ...
  try {
    ...
    const sheet1 = spreadsheet.data.sheets?.find(
      (s) => s.properties?.title === 'Sheet1' || s.properties?.title === '시트1'
    );
...
async function createDashboardTab(
  ...
  if (negativeReviews.length > 0) {
    await setRowHeight(
      spreadsheetId,
      '대시보드',
      negativeReviewStartRow,
      negativeReviewStartRow + negativeReviews.length,
      110
    );
  }
}
 ...
 * 매장별 분석 탭 생성
 */
async function createStoreAnalysisTab(
  spreadsheetId: string,
  aggregation: BrandWeeklyAggregation
): Promise<void> {
 ...
 * 키워드 분석 탭 생성
 */
async function createKeywordAnalysisTab(
  spreadsheetId: string,
  aggregation: BrandWeeklyAggregation
): Promise<void> {
 ...
 * 부정 리뷰 상세 탭 생성
 */
async function createNegativeReviewsTab(
  spreadsheetId: string,
  aggregation: BrandWeeklyAggregation
): Promise<void> {
 ...
 * 플랫폼별 분석 탭 생성
 */
async function createPlatformAnalysisTab(
  spreadsheetId: string,
  aggregation: BrandWeeklyAggregation
): Promise<void> {
 ...
 * 원본 데이터 탭 생성
 */
async function createRawDataTab(
  spreadsheetId: string,
  aggregation: BrandWeeklyAggregation
): Promise<void> {
 ...
 * 브랜드별 주간 리포트 시트 생성
 */
export async function createBrandWeeklyReportSheet(
  aggregation: BrandWeeklyAggregation,
  logger: Logger
): Promise<{ spreadsheetId: string; spreadsheetUrl: string }> {
  // 1. 폴더 생성/조회
  const folderId = await getOrCreateBrandReportFolder(
    aggregation.brandName,
    aggregation.weekLabel
  );
